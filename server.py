...
from graph_tools import AccessGraphEngine
...
# --- Graph & Knowledge Tools (Graphify-like Token Reduction & Visualization) ---

_graph_cache: Dict[str, Tuple[float, AccessGraphEngine]] = {}

def _get_or_build_graph(db_name: str, force_refresh: bool = False) -> AccessGraphEngine:
    """Builds or retrieves cached AccessGraphEngine for the given database.
    Can load directly from pre-computed graph.json (instant 0.005s) or extract from DB."""
    # Check if a JSON file path was directly passed
    if db_name.lower().endswith(".json"):
        json_path = os.path.abspath(db_name)
        if os.path.exists(json_path):
            logger.info(f"Loading graph directly from JSON file: {json_path}")
            return AccessGraphEngine.from_graphify_json(json_path)
    ...
    if not force_refresh and path in _graph_cache:
        cached_mtime, engine = _graph_cache[path]
        if cached_mtime == mtime:
            return engine

    # Check if a pre-existing graphify-out/graph.json exists next to the database
    if not force_refresh:
        candidate_json = os.path.join(os.path.dirname(path), "graphify-out", "graph.json")
        if os.path.exists(candidate_json) and os.path.getmtime(candidate_json) >= mtime:
            try:
                engine = AccessGraphEngine.from_graphify_json(candidate_json)
                _graph_cache[path] = (mtime, engine)
                logger.info(f"Loaded graph in 0.005s from existing {candidate_json}")
                return engine
            except Exception as e:
                logger.debug(f"Could not load pre-existing graph.json: {e}")

    engine = AccessGraphEngine(path)
    extracted = False

    # 1. Primary extraction: DAO.DBEngine (Fastest, in-process, headless, NEVER hangs on GUI modals or AutoExec)
    for engine_progid in ["DAO.DBEngine.120", "DAO.DBEngine.36", "DAO.DBEngine"]:
        try:
            dbe = win32com.client.Dispatch(engine_progid)
            db = dbe.OpenDatabase(path)
            try:
                engine.extract_from_dao(db)
                extracted = True
                logger.info(f"Graph extracted successfully via {engine_progid} for {db_name}")
                break
            finally:
                db.Close()
                del db
            ...
            logger.debug(f"{engine_progid} extraction failed: {e}")

    # 2. Secondary fallback: pyodbc (ODBC driver, also fast and headless)
    if not extracted:
        try:
            driver = get_driver()
            conn_str = f"DRIVER={{{driver}}};DBQ={path};"
            with pyodbc.connect(conn_str) as conn:
                engine.extract_from_pyodbc(conn)
            extracted = True
            logger.info(f"Graph extracted successfully via pyodbc for {db_name}")
        except Exception as e:
            logger.warning(f"pyodbc extraction failed: {e}")

    # 3. Tertiary fallback: Access.Application COM (only if previous headless methods failed)
    if not extracted:
        try:
            def op(access):
                engine.extract_from_dao(access)
                return True
            _with_access_database(db_name, op)
            extracted = True
            logger.info(f"Graph extracted via Access.Application fallback for {db_name}")
        except Exception as e:
            logger.error(f"Access.Application COM fallback also failed: {e}")
            raise Exception(f"Failed to extract database structure: {e}")

    engine.compute_metrics_and_communities()
    _graph_cache[path] = (mtime, engine)
    return engine

@mcp.tool()
def export_graphify_graph(db_name: str, output_path: str = "graphify-out/graph.json") -> dict:
    """Export an MS Access dependency graph in Graphify's native node-link JSON format.

    The result can be supplied directly to Graphify, for example:
    `graphify cluster-only . --graph graphify-out/graph.json`.
    It also creates `index.html` beside the JSON as a dependency-free viewer.
    It contains tables, fields, relationships, queries, forms, reports, controls,
    and VBA references. Every dependency includes evidence explaining why it exists.
    
    Args:
        db_name: Database name or path
        output_path: Path where graph.json will be saved (default: 'graphify-out/graph.json')
        
    Returns:
        dict with extraction statistics, paths to graph.json and index.html
    """
    try:
        engine = _get_or_build_graph(db_name, force_refresh=True)
        graph_data = engine.to_graphify_json()
        metrics = engine.compute_metrics_and_communities()

        # If output_path is relative, resolve it relative to the database directory
        # This prevents saving in C:\Windows\system32 when run from Claude Desktop or Kiro
        if not os.path.isabs(output_path):
            db_dir = os.path.dirname(engine.db_path)
        ...
        # Generate index.html beside the JSON
        html_path = os.path.join(out_dir, "index.html")
        engine.generate_interactive_html(html_path)

        return {
            "success": True,
            "database": os.path.basename(engine.db_path),
            "graph_json_path": os.path.abspath(output_path),
            "interactive_html_path": os.path.abspath(html_path),
            "total_nodes": metrics["total_nodes"],
            "total_edges": metrics["total_edges"],
            "tables": graph_data["metadata"]["total_tables"],
            "relationships": graph_data["metadata"]["total_relationships"],
            "queries": graph_data["metadata"]["total_queries"],
            "forms": graph_data["metadata"]["total_forms"],
            "god_nodes": metrics["god_nodes"],
            "communities_count": metrics["communities_count"],
            "message": f"Successfully exported knowledge graph and interactive viewer to {output_path} and {html_path}"
        }
    except Exception as e:
        logger.error(f"Error exporting graph for {db_name}: {e}")
...
def query_database_context(
    ...
    """Drastically reduce LLM token consumption when adding or modifying database records.
    
    Instead of reading the entire database schema, this tool performs targeted graph
    traversal to return ONLY the relevant tables, 1-hop foreign key neighbors, data types,
    AutoNumber flags, and constraints within a strict token budget.
    
    Args:
        db_name: Database name or path
        task_description: Description of what needs to be added, edited, or queried (e.g. 'Add new invoice line item for product')
        relevant_tables: Optional comma-separated table names to focus on (e.g. 'invoices,itemstrans')
        depth: Neighbor hop distance (default 1 hop for direct foreign keys)
        token_budget: Maximum approximate token budget for the returned schema context (default 1500)
        
    Returns:
        Compact Markdown cheat sheet containing only relevant tables, columns, PK/FK links, and rules
    """
    try:
        engine = _get_or_build_graph(db_name)
        context = engine.get_subgraph_context(
        ...
        logger.error(f"Error querying database context for {db_name}: {e}")
        return f"Error querying database context: {str(e)}"

@mcp.tool()
def get_table_neighborhood(db_name: str, table_name: str, depth: int = 1) -> str:
    """Get a focused neighborhood summary for a specific table: its columns, data types,
    primary and foreign keys, direct relationships (parents and children), and connected queries.
    
    Args:
        db_name: Database name or path
        table_name: The table to inspect
        depth: Neighbor distance (default 1)
        
    Returns:
        Structured summary of the table and its immediate network
    """
    try:
        engine = _get_or_build_graph(db_name)
        return engine.get_subgraph_context(
            task_description=f"Inspect table {table_name}",
            relevant_tables=table_name,
            depth=depth,
            token_budget=2000
        )
    except Exception as e:
        return f"Error getting table neighborhood for '{table_name}': {str(e)}"

@mcp.tool()
def find_relationship_path(db_name: str, source_table: str, target_table: str) -> dict:
    """Find the shortest relationship path between two tables in the database and generate
    the exact MS Access INNER JOIN syntax with proper parentheses.
    
    Args:
        db_name: Database name or path
        source_table: Starting table name (e.g. 'Customers')
        target_table: Destination table name (e.g. 'Products')
        
    Returns:
        dict with path of tables, hop distance, and recommended MS Access JOIN SQL
    """
    try:
        engine = _get_or_build_graph(db_name)
        result = engine.find_path(source_table, target_table)
        return result
    except Exception as e:
        return {"success": False, "error": str(e)}

@mcp.tool()
def generate_interactive_graph_html(db_name: str, output_file: str = "database_graph.html") -> str:
    """Create a standalone, rich interactive HTML visualization showing all objects
    (Tables, Columns, Relationships, Queries, Forms, Reports, Modules) in the database.
    Features force-directed 2D canvas simulation, pan, zoom, search, filtering, and schema drawer.
    
    Args:
        db_name: Database name or path
        output_file: Path to save the interactive HTML file (default: 'database_graph.html')
        
    Returns:
        Success message with the absolute path to the generated HTML file
    """
    try:
        engine = _get_or_build_graph(db_name, force_refresh=True)
        if not os.path.isabs(output_file):
            db_dir = os.path.dirname(engine.db_path)
            output_file = os.path.join(db_dir, output_file)
        output_file = os.path.normpath(output_file)
        abs_path = engine.generate_interactive_html(output_file)
        return f"Interactive database graph generated successfully at: {abs_path}"
    except Exception as e:
        return f"Error generating interactive HTML graph: {str(e)}"
